...
BOOL attack(int sq, int s)
{
	...
	*/
	...
	//Si on est les blanc on commance la boucle à 1, si on est les noirs on commance la boucle à 17
	int indexColor = 1;
	if (s == DARK)
		indexColor = 17;
	...
	for (i = 0; i < 64; ++i)
	{
		if (color[i] == side) {
			if (piece[i] == PAWN) {
				...
				/*
				La partie concerne toutes les autres pièces suaf les pions
				*/
				for (j = 0; j < offsets[piece[i]]; ++j)
					for (n = i;;) {
						n = mailbox[mailbox64[n] + offset[piece[i]][j]];
						...
						gen_push(i, n, 0); // Générer le coup de non capture
						if (!slide[piece[i]])
							break;
					}
		}
